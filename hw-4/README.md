...
## Резолюция открытых вопросов PRD (TBD)

Дефолтные значения, чтобы снять блокеры для дооценки (где возможно — со ссылкой
на принятые ADR).

| # | Вопрос (TBD) | Резолюция |
|---|---|---|
| 1 | Спецификации API Jivo / GreenAPI | Закладываем **анти-коррупционный слой** (канальные адаптеры): единый внутренний формат, уточнение спеки = доработка адаптера, не ядра. Нужны доступы к sandbox обоих API |
| 2 | Идентификаторы клиента/заказа | Первичный ключ — **номер телефона** (всегда есть из WhatsApp/Jivo); вторичные — номер заказа/договора, e-mail. Нормализация телефона в E.164 |
| 3 | Частота индексации БЗ | По §5 PRD: Confluence и материалы — **раз в день, 02:00–04:00**, инкрементально; заказы — **прямой SQL** (не индексируются). Вопрос фактически закрыт |
| 4 | Реестр версий индексов | **Не нужен** — храним только последнюю версию (источник истины — Confluence/ERP, индекс пересобираем). Trade-off: нет аудита «почему ответил так раньше» |
| 5 | Нужен ли долговременный контекст | **Да**, закладываем в архитектуру MVP, но включаемый флагом и ограниченный |
| 6 | TTL/объём долговременного контекста | Дефолт: **TTL 30 дней** (скользящее окно), **≤20 сообщений / ~4–8k токенов** на клиента, хранение **в периметре** с маскированием PII. Точные цифры — на согласование с ИБ |
| 7 | Золотой датасет | Предоставляет заказчик, в объём работ не входит. От нас — согласовать **структуру и методологию оценки** под AC-4 (≥85% / 80%) |
| 8 | Формализация требований ИБ | **Самый критичный** — обосновал [ADR-0001](adr/0001-hosting-llm.md). Нужно зафиксировать применимость **152-ФЗ**, маскирование PII, ролевую модель, Vault, data residency. По умолчанию — строгий режим: ПДн не покидают периметр |
