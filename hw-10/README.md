...
|---|---------|---------------------|-------------------|
| 1 | **Забытые GPU-инстансы** | После A/B две карты A100 оставили круглосуточно; ночные Jupyter не гасились; загрузка ~**15%** | GPU стенды **+450** |
| 2 | **Неэффективное хранение журналов** | Отладочный уровень в бою, сырые промпты, access-лог без выборки, аудит без срока жизни | Хранилище **+240**, наблюдаемость **+160** |
| 3 | **Слишком «умная» модель для простых задач** | ~**40%** FAQ и шаблонных статусов идут в **70B**, хотя хватило бы 8B или кэша | запас GPU в пике, задержка роста HPA |
| 4 | **Стенд = размер боя** | Фиксированные CPU-узлы; staging сопоставим с prod ночью | CPU-кластер **+130** |
| 5 | **Лишний realtime там, где нужен пакетный режим** | Ночная оценка качества и переиндексация крутятся на постоянно включённом GPU-сервисе | часть GPU стендов |
| 6 | **Ежедневная полная переиндексация** | Смещение данных базы ~раз в неделю, а полный пересчёт индекса каждый день | GPU + хранилище снимков |
| 7 | **Копии данных между зонами** | Датасеты и снимки KB копируются между зонами «на всякий случай» | Сеть **+70** |
| 8 | **Нет меток и владельцев затрат** | Нельзя быстро ответить, чей стенд и чей бакет ест бюджет | задержка реакции на счёт |
