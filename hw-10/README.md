...
| 9 | Пакетный режим для ночной оценки качества вместо постоянно включённого realtime-сервиса | Не держим GPU ради редкого пакетного прогона | часть эффекта Spot/стендов | 1–2 нед. |
...
| **Экономия** | насколько уменьшатся затраты: **Сейчас − После мер** |
...
**Стоимость** - такая же целевая метрика системы, как **задержка ответа** (насколько быстро отвечаем) и **опора на документы** (насколько ответ опирается на базу знаний, без выдумок). При архитектурных решениях оцениваем не только «быстрее / качественнее», но и **сколько это стоит в ₽/мес**.
