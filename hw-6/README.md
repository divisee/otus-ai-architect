...
**Кейс:** ритейлер **TechnoMart** (сквозной кейс курса, продолжение ДЗ №1–5).

**Объект QA** — **AI‑ассистент поддержки** TechnoMart: RAG‑бот в мессенджерах (WhatsApp/Jivo) по базе знаний (Confluence) и данным заказов (ERP/1С). RAG‑система → качество меряем RAG‑метриками. Архитектура — в [ДЗ №4](../hw-4/README.md); компоненты безопасности и governance переиспользуем из [ДЗ №2](../hw-2/README.md) и [ДЗ №5](../hw-5/README.md).

**Цель:** спроектировать комплекс обеспечения качества — **Security**, **тестирование RAG** и **Observability** с AI‑метриками. Теория — в [конспекте](summary/README.md).
...
Переиспользуем из прошлых ДЗ: **PII Anonymizer + Token Vault** ([ДЗ №2](../hw-2/README.md)) → основа санитайзера; **self‑hosted LLM в периметре, 152‑ФЗ** ([ДЗ №4](../hw-4/README.md)); **«ПДн не в LLM», DQ, lineage** ([ДЗ №5](../hw-5/README.md)). Достраиваем в этом ДЗ: явные Input/Output **Guardrails** и **Secret Manager**.

### Компоненты защиты

Каждый узел имеет собственный контур проверок (**Defense‑in‑Depth**). Красным — добавленное в этом ДЗ.
...
**Faithfulness** и **Answer Relevancy** считаются **без** ground truth — их можно мерить даже на живом трафике.

### Инструмент: Ragas vs DeepEval

Выбираем между двумя актуальными фреймворками авто‑оценки RAG (оба используют «LLM‑as‑a‑judge» — метрики считает модель‑судья).
...
**Выбор — Ragas** (основной): специализирован под RAG‑метрики из задания, есть практический опыт, метрики ложатся в Langfuse (§3). **DeepEval** — опционально, для safety/red‑teaming и pytest‑gate в CI.
