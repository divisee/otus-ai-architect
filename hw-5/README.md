...
- **Форматы данных.** Сырые данные попадают в Lake в исходном формате (**JSON** — события из Kafka, **XML** — каталог из ERP); события дополнительно сериализуются в **Avro** (компактный бинарный строчный формат со встроенной схемой — удобен для потоковых событий и контроля версий схемы через Schema Registry). После очистки и нормализации (Silver/Gold) данные хранятся в **Parquet** (колоночный формат, эффективен для аналитических и ML-нагрузок), а curated-таблицы — поверх **Delta Lake** (ACID, версии). Признаки online-store — в **Redis**, embeddings — в **Qdrant**.
- **Хранилища под задачи.** Для предотвращения **Training-Serving Skew** используется **Feature Store**: *offline store* (**PostgreSQL** / Parquet) — для обучения на исторических данных, *online store* (**Redis**) — для быстрого получения тех же признаков на инференсе. Берём **самописный Feature Store на PostgreSQL + Redis** (не managed Feast/Tecton): для одной recsys-команды это дешевле и проще в эксплуатации, переиспользует уже выбранный в ДЗ №2 стек и не создаёт vendor lock-in; единая логика фич (`feature_defs`) и materialization обеспечивают одинаковые значения для train и serve. **Vector DB (Qdrant)** хранит embeddings товаров, пользователей и сессий для поиска похожих (content-based и cold-start). Фактические данные (каталог, заказы, остатки, события) — в **Lakehouse / SQL**.
...
> **Materialize** — шаг публикации фич из offline-хранилища в online: посчитанные значения **копируются из PostgreSQL в Redis**, чтобы инференс читал их за миллисекунды, не пересчитывая по истории. Два режима: **batch** (nightly: Gold → FS Offline → Redis для медленных фич вроде `user_purchases_30d`) и **stream** (Spark Streaming инкрементит счётчики в Redis каждые 1–5 мин для быстрых фич вроде `user_clicks_1h`). Поскольку offline и online берут значения из одной логики (`feature_defs`), train и serve видят одинаковые фичи → нет training-serving skew.
...
| **Parquet** | Колоночный формат файлов — эффективен для аналитики и ML (сжатие, чтение нужных колонок) |
| **Avro** | Компактный бинарный **строчный** формат со встроенной схемой; удобен для потоковых событий и эволюции схемы (Schema Registry) |
