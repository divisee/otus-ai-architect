...
- **Форматы данных.** Сырые данные попадают в Lake в исходном формате (**JSON** — события из Kafka, **XML** — каталог из ERP); события дополнительно сериализуются в **Avro** под Schema Registry. После очистки и нормализации (Silver/Gold) данные хранятся в **Parquet** (колоночный формат, эффективен для аналитических и ML-нагрузок), а curated-таблицы — поверх **Delta Lake** (ACID, версии). Признаки online-store — в **Redis**, embeddings — в **Qdrant**.
