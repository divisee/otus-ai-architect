...
**Маршрут выступления.** Регламент — десять минут, колода при этом остаётся полной: часть слайдов показывается только по вопросам. Звезда в правом верхнем углу отмечает слайд основного выступления. Отмечены пятнадцать: обложка, автор, цель и задачи, проблема и ценность (слайды 1–4); архитектура контура, управление диалогом и хранилища, мультиагентная система (6–8); матрица доступа (10); MVP против промышленного контура (16); мониторинг контура (18); стоимость владения и три контура метрик (19–20); результаты и выводы, итог и контакты (22–24). Клавиши `Shift` + `→` и `Shift` + `←` переходят между отмеченными слайдами, обычные стрелки — по всем подряд.
...
| 5 | решение: замкнутый контур, четыре уровня, границы объёма | [Объём системы](#объём-системы), [Стек](#стек) |
| 6–7 | C4 L1, L2 | [Схемы](#схемы) |
| 8 | мультиагентная система: субагенты, инструменты, HITL | [Мультиагентная схема](#мультиагентная-схема), [C4 L3 — Component](#c4-l3--component) |
| 9 | десять решений ADR | [Стек](#стек), [`docs/adr/`](docs/adr/README.md) |
| 10 | матрица доступа к сведениям | [Доступ](#доступ) |
| 11 | обработка персональных данных в реплике | [Персональные данные в реплике](#персональные-данные-в-реплике), [ADR-0010](docs/adr/0010-pii-detection.md) |
| 12 | запись ребёнка с подтверждением | [Sequence](#sequence) |
| 13–14 | онтология и ER | [Онтология и ER](#онтология-и-er) |
| 15 | расчёт нагрузки и выбор видеокарт | [Типовой день и пик](#типовой-день-и-пик), [ADR-0004](docs/adr/0004-gpu-budget.md) |
| 16 | MVP: что проверено на стенде и что будет в промышленном контуре | [Порты стенда](#порты-стенда), [Локальный прогон](#локальный-прогон) |
| 17 | сборка, поставка и обновление контура | [Развёртывание](#развёртывание) |
| 18 | мониторинг промышленного контура | [Мониторинг промышленного контура](#мониторинг-промышленного-контура) |
| 19 | экономика: закупка, владение, цена сессии | [Капитальные затраты](#капитальные-затраты), [Стоимость сессии](#стоимость-сессии) |
| 20 | три контура метрик: онлайн, офлайн, правовой | [Метрики и проверка эффекта](#метрики-и-проверка-эффекта) |
| 21 | A/B против живой линии и цели первого года | [A/B против живой линии](#ab-против-живой-линии), [Цели первого года](#цели-первого-года) |
| 22 | результаты и выводы: что сделано, трудности, время, развитие | [Реестр требований](#реестр-требований), [Локальный прогон](#локальный-прогон), [Планы развития](#планы-развития) |
| 23–24 | итог и контакты | весь документ |
