...
**Маршрут выступления.** Регламент — пять минут, колода при этом остаётся полной: часть слайдов показывается только по вопросам. Звезда в правом верхнем углу отмечает слайд основного выступления. Отмечены четырнадцать: обложка, автор, цель и задачи, проблема и ценность (слайды 1–4); архитектура контура, управление диалогом и хранилища, мультиагентная система (6–8); матрица доступа (11); MVP против промышленного контура (17); стоимость владения и три контура метрик (20–21); результаты и выводы, итог и контакты (24–26). Клавиши `Shift` + `→` и `Shift` + `←` переходят между отмеченными слайдами, обычные стрелки — по всем подряд.
...
| 9 | когнитивная схема пути реплики | [Когнитивная схема](#когнитивная-схема) |
| 10 | десять решений ADR | [Стек](#стек), [`docs/adr/`](docs/adr/README.md) |
| 11 | матрица доступа к сведениям | [Доступ](#доступ) |
| 12 | обработка персональных данных в реплике | [Персональные данные в реплике](#персональные-данные-в-реплике), [ADR-0010](docs/adr/0010-pii-detection.md) |
| 13 | запись ребёнка с подтверждением | [Sequence](#sequence) |
| 14–15 | онтология и ER | [Онтология и ER](#онтология-и-er) |
| 16 | расчёт нагрузки и выбор видеокарт | [Типовой день и пик](#типовой-день-и-пик), [ADR-0004](docs/adr/0004-gpu-budget.md) |
| 17 | MVP: что проверено на стенде и что будет в промышленном контуре | [Порты стенда](#порты-стенда), [Локальный прогон](#локальный-прогон) |
| 18 | CI/CD и ввод в эксплуатацию | [Развёртывание](#развёртывание) |
| 19 | мониторинг промышленного контура | [Мониторинг промышленного контура](#мониторинг-промышленного-контура) |
| 20 | экономика: закупка, владение, цена сессии | [Капитальные затраты](#капитальные-затраты), [Стоимость сессии](#стоимость-сессии) |
| 21 | три контура метрик: онлайн, офлайн, правовой | [Метрики и проверка эффекта](#метрики-и-проверка-эффекта) |
| 22–23 | A/B против живой линии, цели первого года | [A/B против живой линии](#ab-против-живой-линии), [Цели первого года](#цели-первого-года) |
| 24 | результаты и выводы: что сделано, трудности, время, развитие | [Реестр требований](#реестр-требований), [Локальный прогон](#локальный-прогон), [Планы развития](#планы-развития) |
| 25–26 | итог и контакты | весь документ |
