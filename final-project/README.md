...
Годовая стоимость владения: амортизация 3,08 + электроэнергия 0,15 + сопровождение 0,54 ≈ **3,8 млн ₽/год**, что и даёт ≈ 30 ₽ на сессию при 125 000 сессий. Высвобождение линии на устойчивом режиме — ≈ 2,7 млн ₽/год ([Экономия линии](#экономия-линии)). Разница около **1 млн ₽/год** — цена законного канала в периметре.
...
Первые три строки работают как ворота релиза: не прошли — выкладки нет. К выкладке прикладывается отчёт прогона: дата, версия корпуса и правил доступа, вердикт по каждому набору. Стенд без GPU закрывает их 17 автотестами (доступ, опека, обязанности регистратуры, диагноз, HITL, оба канала). Целевой контур добавляет Recall@10 и nDCG на bge-m3.
...
Фаза GPU не блокирует проверку ACL и GraphRAG. Стенд: **17 тестов** (ACL, ReBAC, обязанности регистратуры, диагностика, HITL, оба канала).
...
Критерий стенда без GPU: 17 автотестов. Критерий целевого контура: p95 `/v1/chat` ≤ 8 с при 10 RPS на H100; p95 ASR реплики 5 с ≤ 1,0 с на L40S.

### Локальный прогон

Дата: **2026-09-12**. Хост: macOS, `uvicorn` на `127.0.0.1:8080`. GPU и vLLM не запускались. Генератор — grounded, граф — Cypher-seed в памяти, индекс — лексический. Автотесты: **17 passed**.
...
| 21 | четыре зоны сети | [Deployment](#deployment) |
| 22 | экономика: закупка, владение, цена сессии | [Нагрузка и стоимость](#нагрузка-и-стоимость), [Экономия линии](#экономия-линии) |
| 23–27 | контуры измерения, онлайн, офлайн, A/B, цели года | [Метрики и проверка эффекта](#метрики-и-проверка-эффекта) |
| 28 | результат работы | [Реестр требований](#реестр-требований), [Локальный прогон](#локальный-прогон) |
| 29 | границы стенда и следующий шаг | [Порты стенда](#порты-стенда), [A/B против живой линии](#ab-против-живой-линии) |
