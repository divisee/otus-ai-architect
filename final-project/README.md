...
Система — голосовой ассистент **типового медицинского центра**. Пациент или регистратор запрашивает сведения об услугах, врачах, филиалах, подготовке к исследованиям и записи на приём. Ответ формируется из графа знаний и документов клиники (**GraphRAG**). Запись выполняется через инструменты CRM и только после подтверждения (**HITL**); перенос и отмена объявлены контрактом порта.
...
| [`demo/`](demo/README.md) | демонстрационный стенд на Streamlit: разговор голосом или текстом, разбор решения о доступе, промпты ассистента в [`demo/prompts/`](demo/prompts/) |
...
- **Автоматическая сверка в CI.** [`scripts/check_docs.py`](scripts/check_docs.py) проверяет шесть вещей: числовые факты презентации есть в README; число автотестов совпадает с фактическим; карта слайдов соответствует презентации; каждая упомянутая ADR существует; термин из сноски слайда встречается на самом слайде; промпты демонстрационного стенда приведены в его README дословно. Запускается на каждый push и pull request ([`.github/workflows/ci.yml`](../.github/workflows/ci.yml)).
- **Отчёт прогона к релизу.** Дата, версия корпуса и правил доступа, вердикт по каждому офлайн-набору — основание выкладки ([Офлайн-метрики](#офлайн-метрики)).

Для промышленного контура предусмотрен ещё один шаг, в текущей поставке не реализованный: агент-ревьюер на pull request, который сверяет затронутые модули с README и ADR и отмечает термины без расшифровки в глоссарии.
...
Справочная линия двух филиалов — 500 обращений в день, в пиковый час 100 ([допущения расчёта](#допущения)). Одно справочное обращение занимает у оператора около четырёх минут ([Экономия линии](#экономия-линии)). Слева — состояние линии сегодня, справа — что даёт контур в периметре.


| Сегодня | С ассистентом |
| ------- | ------------- |
| Линия — узкое место: в пиковый час очередь растёт быстрее, чем регистратура успевает её разбирать | Подготовка, филиал и свободное окно закрываются в том же обращении. Цель — закрывать без оператора не менее 40% обращений, принятых ассистентом ([Цели первого года](#цели-первого-года)) |
| Одни и те же вопросы: цена услуги, адрес филиала, подготовка к исследованию, ближайшее окно | Ответ собирается из графа услуг и документов клиники; запись — только после подтверждения. Нет опоры — отказ или оператор |
...
| 5 | Собрать знания клиники: онтологию графа, корпус документов с метками доступа, справочник МКБ-10 | [Онтология и ER](#онтология-и-er), [`backend/data/kb/`](backend/data/kb/manifest.yaml) |
| 5 | Реализовать MVP Control Plane и закрыть сценарии автотестами | [Стенд](#стенд), [Локальный прогон](#локальный-прогон) |
| 6 | Посчитать ёмкость и стоимость владения, задать метрики и план A/B | [Нагрузка и стоимость](#нагрузка-и-стоимость), [Метрики и проверка эффекта](#метрики-и-проверка-эффекта) |

## Владение

Идентификатор контура: `clinic-voice-graphrag`. Дата фиксации: **2026-09-21**. Обновлять при смене модели, Policy или состава GPU.

Все роли контура закрывает один человек — Распутина Анастасия ([divisee](https://github.com/divisee)). В промышленной эксплуатации они разойдутся по людям, поэтому перечислены отдельно: владелец ML и владелец со стороны бизнеса, архитектор, ответственный за обработку ПДн, владелец ACL и Policy, инженер инференса, инженер речевых технологий, владелец вычислительного бюджета, дежурный по инцидентам.

Зоны ответственности в репозитории: ADR и схемы, Control Plane (`backend/`), корпус знаний и МКБ, Data Plane и Compose, расчёт нагрузки и TCO.
...
- промышленная CRM — вне поставки (в стенде синтетический регистр: пациент 1, пациент 2, пациент 3);
- обучение LLM и ASR — вне поставки;
- промышленная АТС — вне поставки (вход: микрофон, wav или транскрипт);
- штатный blue/green и ночная полная переиндексация — вне MVP, предусмотрены в целевом контуре ([Развёртывание](#развёртывание)).
...
Роли в тексте и на слайдах обезличены. В регистре стенда карточки носят синтетические ФИО — это и есть те сведения, которые Policy не отдаёт чужому актору.
...
| ASR, диаризация, TTS | Whisper large-v3-turbo, WhisperX, Silero | [0009](docs/adr/0009-voice-whisper-diarization.md) |


**Control Plane** — API и агенты. **Data Plane** — граф, векторы, модели, заглушка CRM.

Разделение — не дань терминологии, оно окупается тремя вещами.

- **Правила доступа проверяются без видеокарт.** Решение о доступе, отказ в диагнозе и подтверждение записи принимает Control Plane до обращения к модели, поэтому весь 31 автотест идёт на ноутбуке за доли секунды, а GPU нужен только для задержки и качества формулировок ([Порты стенда](#порты-стенда)).
- **Хранилище меняется, правила — нет.** Neo4j вместо графа в памяти, Qdrant вместо BM25, vLLM вместо стендового сборщика — это замена реализации за узким контрактом (`search`, `walk`, `generate`), и ни одно правило Policy при этом не переписывается.
- **Границу видно в сети.** GPU-узлы принимают запросы только от Control Plane и наружу не ходят, а значит «модель не отправит данные во внешний API» — свойство размещения, а не обещание промпта ([ADR-0001](docs/adr/0001-closed-contour.md)).

Обратная сторона — плоскость управления становится единственной точкой, где живут права: ошибка в Policy не компенсируется ничем ниже. Поэтому у неё отдельный узел маршрута, отдельный журнал оснований и [набор инвариантов](backend/README.md#13-инварианты), закрытых автотестами.
...
> [!IMPORTANT]
> Отклонённый фрагмент **в контекст модели не передаётся**. Канал на ACL **не влияет**. `actor_id` — учётная запись или АОН, **не** `SPEAKER_`*.
...
L1 дан обычным `flowchart`: синтаксис `C4Context` на GitHub не рендерится.
...
| CPU-узлы     | API, LangGraph, Neo4j, Qdrant, Postgres, Langfuse |
...
| Vault    | пароли, токены            | JIT, не `.env` на диске промышленных узлов |
...
Сам код система никогда не выводит и не подбирает: она только читает уже выданное направление ([ADR-0001](docs/adr/0001-closed-contour.md), раздел «Ограничения»). Полный МКБ-10: [backend/data/kb/reference/mkb10-parsed.csv](backend/data/kb/reference/mkb10-parsed.csv). В промышленном контуре — выгрузка НСИ Минздрава `1.2.643.5.1.13.13.11.1005`.
...
> [!WARNING]
> Потолок **30 сессий/мин** — инженерный запас, не штат двух филиалов. 90 ASR/мин при p95 = 1,0 с превышает ~60 последовательных слотов L40S. Штатный пик **1,7 сессии/мин**: 30 ÷ 1,7 ≈ 18, в расчёте принят консервативный ориентир запаса **15×**.

Второй H100 в расчёте пропускной способности не участвует: это горячий резерв / вторая реплика (ADR-0004). Расчётный бюджет одного H100 — 10 RPS текста при p95 ≤ 8 с; инженерный потолок контура требует 3 RPS, то есть 30% бюджета.

**Запас по видеокартам в штатном режиме.** H100 под генерацией занят на 2% по вычислениям и на 40–55 ГБ из 80 по памяти — свободно 98% вычислений и 25–40 ГБ. L40S под речью, эмбеддингами и малой моделью занят на 8% и на 18–24 ГБ из 48 — свободно 92% и двукратный запас по памяти. Второй H100 свободен целиком. По сессиям запас пятнадцатикратный: штатный пик 1,7 сессии/мин против расчётного потолка 30.
...
Мультиагентность в расчёте учтена: узлы графа состояний ходят к моделям разного размера. Маршрут с вызовом инструментов и генерация ответа — 32B на H100; вспомогательные шаги субагентов (переписывание запроса, проверка опоры на выходе) — T-lite-it-2.1 8B FP8 на L40S, ≈ 8–10 ГБ рядом с распознаванием и эмбеддингами. Архитектура и токенизатор те же, что у основной модели, поэтому промпты, парсер вызова инструментов и рецепт LoRA общие (ADR-0003). Резерв на рост нагрузки — Qwen3-30B-A3B: 3,3 млрд активных параметров из 30,5 дают большую пропускную способность на тех же H100.

Policy, обход графа и BM25 модель не вызывают вовсе — это код, а не агент. Поэтому рост числа субагентов не увеличивает нагрузку на H100 линейно.
...
Третья карта нужна по профилю нагрузки, а не про запас: распознавание и синтез — короткие частые запросы, их всплеск совпадает с пиком звонков и на общей карте вытесняет кэш внимания генерации. Перенос речи на второй H100 лишает контур горячего резерва, а третий H100 под 8% загрузки стоил бы 2,5–4,0 млн ₽ вместо 0,8–1,5 млн ₽.

L40S выбран среди карт под речь по памяти и форм-фактору: узлу нужно 18–24 ГБ, потому что распознавание, диаризация, синтез, эмбеддинги и малая модель субагентов работают одновременно. T4 (16 ГБ) не хватает памяти; L4 и A10 (24 ГБ) не оставляют запаса, у A10 нет FP8; RTX 4090 (24 ГБ) без ECC и с охлаждением не под стойку; RTX 6000 Ada подходит по характеристикам, но отклонена по форм-фактору. Подробный отбор — [ADR-0004](docs/adr/0004-gpu-budget.md).
...
Потребление: 2×350 + 350 + 2×250 = 1,55 кВт.

1,55 кВт × 12 ₽ × 24 × 365 ≈ 163 тыс. ₽/год.


| Статья                                                            | Оценка, тыс. ₽/год |
| ----------------------------------------------------------------- | ------------------ |
| Электроэнергия GPU-контура                                        | 163                |
| Сопровождение 0,3 ставки (ориентир ФОТ с налогами 1,8 млн/ставка) | 540                |
| **Итого OpEx без амортизации**                                    | **703**            |
...
Годовая стоимость владения: амортизация 3,08 + электроэнергия 0,16 + сопровождение 0,54 = **3,78 млн ₽/год**; 3,78 млн ₽ ÷ 125 000 сессий = **30,3 ₽/сессия**. Высвобождение линии на устойчивом режиме — ≈ 2,7 млн ₽/год ([Экономия линии](#экономия-линии)). Разница — **≈ 1,1 млн ₽/год**: цена законного канала в периметре.

**Окупаемость и ROI.** Простой срок возврата закупки: 9,25 млн ₽ ÷ 2,16 млн ₽ чистой экономии ФОТ = **4,3 года**. Высвобождение линии возвращает **71%** годовой стоимости владения (2,7 из 3,78 млн ₽), то есть годовой ROI отрицательный — **−29%**: контур дороже эффекта на 1,1 млн ₽ в год.

Это не дефект расчёта, а суть решения. Облачный ассистент дал бы 4–8 ₽ за сессию против 30 и окупался бы быстро, но речь и карточки нельзя передавать наружу (152-ФЗ, 323-ФЗ ст. 13). Непокрытый остаток 1,1 млн ₽ — цена соответствия закону, а не упущенная выгода. Безубыточность по прямому эффекту наступила бы при доле без оператора около 56% вместо целевых 40%; такой цели на первый год не ставится.
...
**Откуда взяты пороги.** Основание каждого:
...
- **5 минут** — окно усреднения: при сборе метрик раз в 30 секунд это десять точек. Более короткое окно срабатывает на одиночном всплеске, более длинное показывает деградацию позже, чем её замечают пациенты;
- **1,0 секунда распознавания** — реплика длиной 5 с при RTF ≤ 0,2 ([ADR-0009](docs/adr/0009-voice-whisper-diarization.md));
- **2 секунды ожидания в очереди** — четверть бюджета ответа: сверх этого очередь расходует время, отведённое на генерацию;
...
Правила дежурства: алерты уходят в Alertmanager и дальше дежурному. Критичные — чужой фрагмент в контексте, отказ CRM, недоступность vLLM — поднимают инженера немедленно, остальные разбираются в рабочие часы. Дашборд Grafana собран из четырёх рядов, по одному на группу; отдельная панель показывает соотношение веток маршрута (знания / запись / отказ): по ней смена поведения пользователей видна раньше, чем по продуктовым метрикам.
...
Правовой контур **тоже наблюдаем**, но коридора допустимых значений у него нет: любое отклонение означает остановку и разбор, а не план улучшений на квартал.
...
Первые три строки работают как ворота релиза: не прошли — релиза нет. К релизу прикладывается отчёт прогона: дата, версия корпуса и правил доступа, вердикт по каждому набору. Стенд разработки закрывает их 31 автотестами (доступ, опека, обязанности регистратуры, диагноз, HITL, обнаружение ПДн, оба канала, ранжирование BM25). Целевой контур добавляет Recall@10 и nDCG на bge-m3.

### A/B против живой линии

Принято. Часть входящих остаётся на операторе (контроль). Часть ведёт ассистент с эскалацией (испытание). Деление устойчивое — по учётной записи или номеру: один человек не переходит между группами внутри периода.
...
Ограничение: на стенде разработки A/B не проводится — там фиксируется контракт доступа.

### Экономия линии

Допущения совпадают с разделом нагрузки: 500 сессий/день, 250 дней, ориентир ФОТ с налогами **1,8 млн ₽/ставка**, среднее время типового справочного звонка **4 мин**, полезная загрузка оператора на линии **8 ч из 10-часовой смены** (остальное — перерывы и оформление).


| Статья | Счёт | Ориентир на устойчивом режиме |
| ------ | ---- | ----------------------------- |
| Закрыто без оператора | 500 × 0,70 × 0,40 = 140 сессий/день | 140 × 4 мин = 9,3 ч/день ÷ 8 ч ≈ **1,2 ставки** |
...
Облачный Speech/LLM даёт 4–8 ₽/сессия и отклоняется: дешевле по оборудованию, но не проходит 152-ФЗ. Замкнутый контур — ≈ 30 ₽/сессия. Закупка окупается законным каналом и высвобождением линии, а не минимальной ценой токена.
...
Обучение и адаптация выполняются внутри периметра: корпус клиники и запросы пациентов не покидают контур (ADR-0001). Правило доступа при любой адаптации не меняется, офлайн-ворота проходятся заново.
...
| R10     | HITL на запись и отмену                      | запись — принято; отмена — контракт порта    |
...
| R14–R15 | C4, Deployment, Data Flow, Sequence, ER      | принято, [Схемы](#схемы)                     |
...
| ------- | --------------- | ---------- |
| Стенд (MVP) | `docker compose --profile data up -d` + `uvicorn app.main:app` | Neo4j, Qdrant, PostgreSQL в контейнерах; Control Plane на хосте; корпус и граф загружаются при старте; `pytest -q` — 31 автотест |
| Сборка | GitHub Actions | сегодня: автотесты и сверка документации на push в `main` и на pull request. В целевом конвейере добавляются линтер, офлайн-ворота релиза, сборка образа и публикация в приватный реестр клиники; версия образа фиксируется вместе с версией корпуса и правил доступа |
...
Terraform в поставку не входит: инфраструктура клиники — bare-metal в собственном периметре, описание кодом окупается только при переводе контура в частное облако.
...
Ответ формулирует один из трёх исполнителей, переключается в интерфейсе: **Ollama** на этом же хосте (по умолчанию, если поднята; модель `gemma3:27b`, 17 ГБ на диске), **сборщик из репозитория** без сети и ключей или **OpenRouter**. Проверка прав и guardrails при любом выборе остаются в коде: отказ в диагнозе и отказ по доступу формулирует не модель. Внешний API допустим только на стенде, где данные синтетические; в промышленном контуре он запрещён ([ADR-0001](docs/adr/0001-closed-contour.md)).

Промпт вынесен в файлы [`demo/prompts/`](demo/prompts/) и приведён дословно в [demo/README.md](demo/README.md): роль и манера речи — в общей части, добавка канала — отдельно, потому что в звонке ответ читают вслух, а в чате глазами. Правил доступа в промпте нет намеренно: отказы формулирует код, инструкцией их не переубедить. Сверку README с файлами промптов делает [`scripts/check_docs.py`](scripts/check_docs.py).

Речевой канал работает локально и в обе стороны: распознавание — faster-whisper на CPU (тот же Whisper из [ADR-0009](docs/adr/0009-voice-whisper-diarization.md), только маленький чекпоинт), ответ озвучивается штатным голосом macOS. Звук с машины не уходит. Подробности — [demo/README.md](demo/README.md).

### Порты стенда

Control Plane в обоих столбцах один и тот же — меняются только порты Data Plane. Решение о доступе, отказ в диагнозе и подтверждение записи принимает код до обращения к модели, поэтому воспроизводятся без видеокарт; на GPU проверяются задержка и качество формулировок.


| Слой           | Промышленный порт  | Порт стенда разработки                        |
| -------------- | ------------------ | --------------------------------------------- |
| Генерация      | vLLM, T-pro-it-2.1 | сборщик по допущенным узлам и чанкам; в демо — локальная модель через Ollama |
...
| ASR / TTS      | Whisper / Silero   | вход — транскрипт; в демо — faster-whisper на CPU и голос macOS |


Стенд закрывает **31 тестом** проверку ACL, ReBAC, обязанностей регистратуры, отказа в диагнозе, HITL, обнаружения ПДн, обоих каналов и ранжирования BM25 — фаза GPU для этого не нужна.
...
Критерий стенда разработки: 31 автотест. Критерий целевого контура: p95 `/v1/chat` ≤ 8 с при 10 RPS на H100; p95 ASR реплики 5 с ≤ 1,0 с на L40S.

### Локальный прогон

Дата: **2026-09-21**. Хост: MacBook Pro, Apple M5 Pro, 48 ГБ, macOS 26.6; Python 3.14, `uvicorn` на `127.0.0.1:8080`, демонстрационный стенд на `8501`. GPU и vLLM не запускались. Генератор — grounded, граф — Cypher-seed в памяти, индекс — BM25. Автотесты: **31 passed**.

Запросы ниже приведены дословно и воспроизводятся: в регистре стенда карточки носят синтетические ФИО, и маршрутизатор узнаёт субъекта именно по ним, а не по обезличенным «пациент 1» из текста документа.

Речевой канал замерен на той же машине, реплика 6,9 с: распознавание faster-whisper на CPU в `int8`, `language=ru`, с фильтром тишины — чекпоинт `base` 0,45 с (RTF 0,06), `small` 1,37 с (RTF 0,20) при целевом RTF ≤ 0,2. Синтез — `say` с русским голосом macOS Milena.

**Пациент 1**, `X-Actor-Id: patient:anna`. «Когда УЗИ у сына и что за код N28.1?»

- намерение — справка, субъект — пациент 2, основание — опека, отказа нет;
- в тексте: N28.1 «Киста почки, приобретенная (не диагноз)», USI-02 3000 ₽, врач УЗИ, подготовка 5 ч, филиал Лесная, направление на 18.11.2026;
- чанк карты пациента 2.

**Пациент 3**, `X-Actor-Id: patient:boris`. «Какое направление у Соколовой Анны?»

```text
Эти сведения закрыты политикой доступа. Могу помочь с публичными услугами или вашей картой.
```

- отказ доступа, субъект — пациент 1; чанк его направления в ответ не вошёл.

**Пациент 1**, `X-Actor-Id: patient:anna`. «Болит живот, что это за диагноз?»
...
Слайды — [presentation.html](presentation.html), опубликованы на https://divisee.github.io/otus-ai-architect/. Схемы — [slides/](slides/), исходники Mermaid лежат рядом с SVG. Каждый слайд опирается на раздел этого документа; расшифровки сокращений идут строкой внизу слайда, сводный список — [GLOSSARY.md](GLOSSARY.md).

**Маршрут выступления.** Регламент — десять минут, колода при этом остаётся полной: часть слайдов показывается только по вопросам. Звезда в правом верхнем углу отмечает слайд основного выступления. Отмечены пятнадцать: обложка, автор, цель и задачи, проблема и ценность (слайды 1–4); архитектура контура, управление диалогом и хранилища, мультиагентная система (5–7); матрица доступа (9); MVP против промышленного контура (15); мониторинг контура (17); стоимость владения и три контура метрик (18–19); результаты и выводы, итог и контакты (21–23). Клавиши `Shift` + `→` и `Shift` + `←` переходят между отмеченными слайдами, обычные стрелки — по всем подряд.


| Слайды | Тема | Раздел README |
| ------ | ---- | ------------- |
| 1–2 | обложка, автор работы | заголовок работы, [Владение](#владение) |
| 3 | цель, задачи и план выступления | [Цель и задачи](#цель-и-задачи), [Содержание](#содержание) |
| 4 | проблема клиники и что закрывает ассистент | [Зачем это клинике](#зачем-это-клинике), [Экономия линии](#экономия-линии) |
| 5–6 | C4 L1, L2 | [Схемы](#схемы) |
| 7 | мультиагентная система: субагенты, инструменты, HITL | [Мультиагентная схема](#мультиагентная-схема), [C4 L3 — Component](#c4-l3--component) |
| 8 | десять решений ADR | [Стек](#стек), [`docs/adr/`](docs/adr/README.md) |
| 9 | матрица доступа к сведениям | [Доступ](#доступ) |
| 10 | обработка персональных данных в реплике | [Персональные данные в реплике](#персональные-данные-в-реплике), [ADR-0010](docs/adr/0010-pii-detection.md) |
| 11 | запись ребёнка с подтверждением | [Sequence](#sequence) |
| 12–13 | онтология и ER | [Онтология и ER](#онтология-и-er) |
| 14 | расчёт нагрузки и выбор видеокарт | [Типовой день и пик](#типовой-день-и-пик), [ADR-0004](docs/adr/0004-gpu-budget.md) |
| 15 | MVP: что проверено на стенде и что будет в промышленном контуре | [Порты стенда](#порты-стенда), [Локальный прогон](#локальный-прогон) |
| 16 | сборка, поставка и обновление контура | [Развёртывание](#развёртывание) |
| 17 | мониторинг промышленного контура | [Мониторинг промышленного контура](#мониторинг-промышленного-контура) |
| 18 | экономика: закупка, владение, цена сессии | [Капитальные затраты](#капитальные-затраты), [Стоимость сессии](#стоимость-сессии) |
| 19 | три контура метрик: онлайн, офлайн, правовой | [Метрики и проверка эффекта](#метрики-и-проверка-эффекта) |
| 20 | A/B против живой линии и цели первого года | [A/B против живой линии](#ab-против-живой-линии), [Цели первого года](#цели-первого-года) |
| 21 | результаты и выводы: что сделано, трудности, время, развитие | [Реестр требований](#реестр-требований), [Локальный прогон](#локальный-прогон), [Планы развития](#планы-развития) |
| 22–23 | итог и контакты | весь документ |
