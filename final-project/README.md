# Платформа голосового ИИ-ассистента типового медицинского центра

On-premise мультиагентный ассистент: **GraphRAG**, разграничение доступа, **152-ФЗ** и врачебная тайна.

> **Система не ставит диагноз.** Ответы опираются на услуги, врачей, подготовку и филиалы. Голос и текст идут в **один граф состояний**. **Внешние API LLM и STT не используются.**

| Назначение | Путь |
|------------|------|
| Этот документ | описание системы, расчёты, все схемы |
| Решения по стеку | [`docs/adr/`](docs/adr/README.md) |
| Код Control Plane | [`backend/`](backend/README.md) |
| Compose Data Plane | [`infra/`](infra/README.md) |
...
3. [Рамка MVP](#рамка-mvp)
    1. [Состав MVP](#состав-mvp)
    2. [Вне объёма](#вне-объёма)
    3. [Ограничения контура](#ограничения-контура)
4. [Роли](#роли)
5. [Стек](#стек)
6. [Доступ](#доступ)
    1. [Правило Policy](#правило-policy)
    2. [Карточки и граф](#карточки-и-граф)
7. [Внешние порты](#внешние-порты)
    1. [Телефония](#телефония)
    2. [МИС](#мис)
8. [Стенд](#стенд)
    1. [Запуск](#запуск)
    2. [Порты стенда](#порты-стенда)
9. [Нагрузка и стоимость](#нагрузка-и-стоимость)
    ...
    6. [Сценарий нагрузки для прогона](#сценарий-нагрузки-для-прогона)
...
12. [Глоссарий терминов и сокращений](#глоссарий-терминов-и-сокращений)
...
- постановка диагноза и подбор МКБ по симптомам;
- промышленная МИС (в стенде — заглушка Анна / Михаил / Борис);
- обучение LLM и ASR;
- отдельный чат на сайте;
...
Контракт: `list_slots`, `book` / `reschedule` / `cancel`, `get_patient` **после Policy**. Заглушка — [`backend/data/crm/seed.json`](backend/data/crm/seed.json).
...
| **Data Plane** | Граф, векторы, модели, сессии, заглушка МИС |
| **FP8** | 8-битный формат с плавающей точкой на H100; режим генерации T-pro |
| **GraphRAG** | Ответ с опорой на обход графа и векторный поиск, не только на веса модели |
...
| **МИС** | Медицинская информационная система / расписание; не sales-CRM |
