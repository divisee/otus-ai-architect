...
| ------- | --------------- | ---------- |
| Стенд (MVP) | `docker compose --profile data up -d` + `uvicorn app.main:app` | Neo4j, Qdrant, PostgreSQL в контейнерах; Control Plane на хосте; корпус и граф загружаются при старте; `pytest -q` — 31 автотестов |
...
Стенд закрывает **31 тестами** проверку ACL, ReBAC, обязанностей регистратуры, отказа в диагнозе, HITL, обнаружения ПДн, обоих каналов и ранжирования BM25 — фаза GPU для этого не нужна.
...
Критерий стенда разработки: 31 автотестов. Критерий целевого контура: p95 `/v1/chat` ≤ 8 с при 10 RPS на H100; p95 ASR реплики 5 с ≤ 1,0 с на L40S.
