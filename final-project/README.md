...
- [17. Стенд](#стенд)
  ...
  - [17.3. Порты стенда](#порты-стенда)
  - [17.4. Сценарий нагрузки для прогона](#сценарий-нагрузки-для-прогона)
  - [17.5. Локальный прогон](#локальный-прогон)
...
2. **Именованные сущности** — ФИО, адрес, организация; модель распознавания для русского языка, веса с внутреннего зеркала. Предопределённые распознаватели Presidio рассчитаны на англоязычные сущности и не применяются: язык подключается отдельной конфигурацией.
...
| ------- | --------------- | ---------- |
| Стенд (MVP) | `docker compose --profile data up -d` + `uvicorn app.main:app` | Neo4j, Qdrant, PostgreSQL в контейнерах; Control Plane на хосте; корпус и граф загружаются при старте; `pytest -q` — 23 автотеста |
...
Фаза GPU не блокирует проверку ACL и GraphRAG. Стенд: **23 теста** (ACL, ReBAC, обязанности регистратуры, диагностика, HITL, оба канала, BM25).
...
Критерий стенда без GPU: 23 автотеста. Критерий целевого контура: p95 `/v1/chat` ≤ 8 с при 10 RPS на H100; p95 ASR реплики 5 с ≤ 1,0 с на L40S.

### Локальный прогон

Дата: **2026-09-12**. Хост: macOS, `uvicorn` на `127.0.0.1:8080`. GPU и vLLM не запускались. Генератор — grounded, граф — Cypher-seed в памяти, индекс — BM25. Автотесты: **23 passed**.
...
**Маршрут выступления.** Регламент защиты — пять минут, колода при этом остаётся полной: часть слайдов показывается только по вопросам. Звезда в правом верхнем углу отмечает слайд основного выступления. Отмечены тринадцать: обложка, автор, содержание, цель и задачи, проблема и ценность (слайды 1–5); архитектура контура, управление диалогом и хранилища, мультиагентная система (7–9); стоимость владения и три контура метрик (24–25); результаты, выводы и контакты (30–32). Клавиши `Shift` + `→` и `Shift` + `←` переходят между отмеченными слайдами, обычные стрелки — по всем подряд.
