...
| ------ | ------- | ------------ |
| Задержка по этапам | p50/p95/p99 ответа `/v1/chat`; p95 распознавания; время обхода графа и поиска; время генерации | p95 ответа > 8 с пять минут подряд; p95 распознавания > 1,0 с |
...
| 8–10 | C4 L1, L2, L3 | [Схемы](#схемы) |
| 11 | оркестратор, субагенты и инструменты | [Мультиагентная схема](#мультиагентная-схема) |
...
| 15 | девять решений ADR | [Стек](#стек), [`docs/adr/`](docs/adr/README.md) |
...
| 24 | поставка: стенд, CI/CD, кластер | [Развёртывание](#развёртывание) |
| 25 | мониторинг после выката | [Мониторинг после выката](#мониторинг-после-выката) |
| 26 | экономика: закупка, владение, цена сессии | [Капитальные затраты](#капитальные-затраты), [Стоимость сессии](#стоимость-сессии) |
| 27–31 | контуры измерения, онлайн, офлайн, A/B, цели года | [Метрики и проверка эффекта](#метрики-и-проверка-эффекта) |
| 32 | результат работы | [Реестр требований](#реестр-требований), [Локальный прогон](#локальный-прогон) |
| 33–34 | итог и контакты | весь документ |
