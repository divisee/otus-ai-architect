...
- **Автоматическая сверка.** [`scripts/check_docs.py`](scripts/check_docs.py) проверяет, что числовые факты презентации есть в README, что число автотестов совпадает с фактическим, что карта слайдов соответствует презентации, что каждая упомянутая ADR существует, что термин из сноски слайда встречается на самом слайде и что промпты демонстрационного стенда приведены в его README дословно. Скрипт запускается в CI на каждый push и pull request.
- **Агент-ревьюер на merge.** На pull request поднимается агент со скиллом проекта: он читает изменения pull request, сверяет затронутые модули с README и ADR и оставляет комментарий, если код разошёлся с документацией или в поставке появился термин без расшифровки в глоссарии.
...
Формулировать ответ может любой из трёх исполнителей, переключается в интерфейсе: **Ollama** на этом же хосте (выбирается по умолчанию, если поднята), **сборщик из репозитория** без сети и ключей или **OpenRouter**. Guardrails и проверка прав остаются в коде при любом выборе: отказ в диагнозе и отказ по доступу формулирует не модель. Внешний API допустим только на стенде — данные синтетические; в промышленном контуре он запрещён ([ADR-0001](docs/adr/0001-closed-contour.md)).

Промпт ассистента вынесен в файлы [`demo/prompts/`](demo/prompts/) и приведён дословно в [demo/README.md](demo/README.md): роль и манера речи — в общей части, добавка канала — отдельно, потому что в звонке ответ читают вслух, а в чате глазами. Правил доступа в промпте нет намеренно: отказы формулирует код до обращения к модели, инструкцией их не переубедить. Сверка README с файлами промптов включена в [`scripts/check_docs.py`](scripts/check_docs.py).

Речевой канал работает локально: распознавание — faster-whisper на CPU (тот же Whisper из [ADR-0009](docs/adr/0009-voice-whisper-diarization.md), только маленький чекпоинт), синтез — штатный голос macOS. Подробности — [demo/README.md](demo/README.md).
...
**Маршрут выступления.** Регламент — пять минут, колода при этом остаётся полной: часть слайдов показывается только по вопросам. Звезда в правом верхнем углу отмечает слайд основного выступления. Отмечены пятнадцать: обложка, автор, содержание, цель и задачи, проблема и ценность (слайды 1–5); архитектура контура, управление диалогом и хранилища, мультиагентная система (7–9); матрица доступа (12); MVP против промышленного контура (20); стоимость владения и три контура метрик (23–24); результаты и выводы, итог и контакты (27–29). Клавиши `Shift` + `→` и `Shift` + `←` переходят между отмеченными слайдами, обычные стрелки — по всем подряд.
...
| 10 | когнитивная схема пути реплики | [Когнитивная схема](#когнитивная-схема) |
| 11 | десять решений ADR | [Стек](#стек), [`docs/adr/`](docs/adr/README.md) |
| 12 | матрица доступа к сведениям | [Доступ](#доступ) |
| 13 | обработка персональных данных в реплике | [Персональные данные в реплике](#персональные-данные-в-реплике), [ADR-0010](docs/adr/0010-pii-detection.md) |
| 14 | запись ребёнка с подтверждением | [Sequence](#sequence) |
| 15–17 | онтология, ER, потоки данных | [Онтология и ER](#онтология-и-er), [Data Flow](#data-flow) |
| 18 | четыре зоны сети | [Deployment](#deployment), [Хранилища и очереди](#хранилища-и-очереди) |
| 19 | расчёт нагрузки и выбор видеокарт | [Типовой день и пик](#типовой-день-и-пик), [ADR-0004](docs/adr/0004-gpu-budget.md) |
| 20 | MVP: что проверено на стенде и что будет в промышленном контуре | [Порты стенда](#порты-стенда), [Локальный прогон](#локальный-прогон) |
| 21 | CI/CD и ввод в эксплуатацию | [Развёртывание](#развёртывание) |
| 22 | мониторинг промышленного контура | [Мониторинг промышленного контура](#мониторинг-промышленного-контура) |
| 23 | экономика: закупка, владение, цена сессии | [Капитальные затраты](#капитальные-затраты), [Стоимость сессии](#стоимость-сессии) |
| 24 | три контура метрик: онлайн, офлайн, правовой | [Метрики и проверка эффекта](#метрики-и-проверка-эффекта) |
| 25–26 | A/B против живой линии, цели первого года | [A/B против живой линии](#ab-против-живой-линии), [Цели первого года](#цели-первого-года) |
| 27 | результаты и выводы: что сделано, трудности, время, развитие | [Реестр требований](#реестр-требований), [Локальный прогон](#локальный-прогон), [Планы развития](#планы-развития) |
| 28–29 | итог и контакты | весь документ |
