...
- [10. Схемы](#схемы)
  ...
  - [10.8. Deployment](#deployment)
  - [10.9. Онтология и ER](#онтология-и-er)
...
- [12. Метрики и проверка эффекта](#метрики-и-проверка-эффекта)
  - [12.1. Онлайн-метрики](#онлайн-метрики)
  - [12.2. Офлайн-метрики](#офлайн-метрики)
  - [12.3. A/B против живой линии](#ab-против-живой-линии)
  - [12.4. Экономия линии](#экономия-линии)
  - [12.5. Цели первого года](#цели-первого-года)
- [13. Планы развития](#планы-развития)
- [14. Реестр требований](#реестр-требований)
- [15. Стенд](#стенд)
  - [15.1. Запуск](#запуск)
  - [15.2. Развёртывание](#развёртывание)
  - [15.3. Порты стенда](#порты-стенда)
  - [15.4. Сценарий нагрузки для прогона](#сценарий-нагрузки-для-прогона)
  - [15.5. Локальный прогон](#локальный-прогон)
- [16. Презентация](#презентация)
- [17. Глоссарий терминов и сокращений](GLOSSARY.md)
...
Мультиагентность в расчёте учтена: узлы графа состояний ходят к моделям разного размера. Тяжёлые шаги — маршрут с вызовом инструментов и генерация ответа — идут в 32B на H100; вспомогательные шаги субагентов (переписывание запроса перед поиском, проверка опоры в выходных guardrails) — в T-lite 8B FP8 на L40S, ≈ 8–10 ГБ рядом с распознаванием и эмбеддингами (ADR-0003). Policy, обход графа и BM25 модель не вызывают вовсе — это код, а не агент, поэтому рост числа субагентов не увеличивает нагрузку на H100 линейно.
...
| 12 | оркестратор, субагенты и инструменты | [Мультиагентная схема](#мультиагентная-схема) |
| 13–14 | каналы и четыре ручки API | [Внешние порты](#внешние-порты), [Стенд](#стенд) |
| 15 | когнитивная схема пути реплики | [Когнитивная схема](#когнитивная-схема) |
| 16–18 | правило доступа, матрица доступа, запись с подтверждением | [Доступ](#доступ), [Sequence](#sequence) |
| 19–21 | онтология, ER, потоки данных | [Онтология и ER](#онтология-и-ер), [Data Flow](#data-flow) |
| 22 | четыре зоны сети | [Deployment](#deployment) |
| 23 | расчёт нагрузки и выбор видеокарт | [Типовой день и пик](#типовой-день-и-пик), [ADR-0004](docs/adr/0004-gpu-budget.md) |
| 24 | поставка: стенд, CI/CD, кластер | [Стенд](#стенд), [infra/](infra/README.md) |
