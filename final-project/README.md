...
**Маршрут выступления.** Регламент — пять минут, колода при этом остаётся полной: часть слайдов показывается только по вопросам. Звезда в правом верхнем углу отмечает слайд основного выступления. Отмечены шестнадцать: обложка, автор, содержание, цель и задачи, проблема и ценность (слайды 1–5); архитектура контура, управление диалогом и хранилища, мультиагентная система (7–9); матрица доступа (14); MVP против промышленного контура (22); стоимость владения и три контура метрик (25–26); результаты, выводы о работе, итог и контакты (29–32). Клавиши `Shift` + `→` и `Shift` + `←` переходят между отмеченными слайдами, обычные стрелки — по всем подряд.
...
| 10–11 | каналы и четыре метода API | [Внешние порты](#внешние-порты), [Стенд](#стенд) |
...
| 17–19 | онтология, ER, потоки данных | [Онтология и ER](#онтология-и-er), [Data Flow](#data-flow) |
...
| 22 | MVP: что проверено на стенде и что будет в промышленном контуре | [Порты стенда](#порты-стенда), [Локальный прогон](#локальный-прогон) |
| 23 | CI/CD и ввод в эксплуатацию | [Развёртывание](#развёртывание) |
| 24 | мониторинг промышленного контура | [Мониторинг промышленного контура](#мониторинг-промышленного-контура) |
| 25 | экономика: закупка, владение, цена сессии | [Капитальные затраты](#капитальные-затраты), [Стоимость сессии](#стоимость-сессии) |
| 26 | три контура метрик: онлайн, офлайн, правовой | [Метрики и проверка эффекта](#метрики-и-проверка-эффекта) |
| 27–28 | A/B против живой линии, цели первого года | [A/B против живой линии](#ab-против-живой-линии), [Цели первого года](#цели-первого-года) |
| 29 | результат работы | [Реестр требований](#реестр-требований), [Локальный прогон](#локальный-прогон) |
| 30 | выводы о работе: цель, трудности, время, развитие | [Цель и задачи](#цель-и-задачи), [Планы развития](#планы-развития) |
