...
- [10. Стенд](#стенд)
  ...
  - [10.4. Локальный прогон](#локальный-прогон)
- [11. Глоссарий терминов и сокращений](GLOSSARY.md)
...
- промышленная CRM (в стенде — заглушка Анна / Михаил / Борис);
- обучение LLM и ASR;
...
- Борис не видит направление Анны;
- Анна видит карту Миши;
...
Борис: «Какое направление у Анны Соколовой и когда её УЗИ?»

- фильтры ввода не блокируют;
- Knowledge находит чанк;
- Policy исключает чужой `acl`;
- CRM по чужому `subject_id` не вызывается;
- ответ — отказ, в трассировке `acl_denied`.

Анна: «Когда УЗИ у сына и что за код N28.1?»

- `subject=misha` по `GUARDIAN_OF`;
- Мише 8 лет — карта доступна;
- граф: USI-02 → подготовка → Морозова → Лесная;
- `IcdCode:N28.1` — справочник, не диагноз.

Борис с текстом про «сына Соколовой» — нет ребра, отказ.
...
- `intent=knowledge`, `subject_id=misha`, `acl_via=guardian`, `acl_denied=false`;
- в тексте: N28.1 «Киста почки, приобретенная (не диагноз)», USI-02 3000 ₽, Морозова А.П., подготовка 5 ч, филиал Лесная, направление на 18.09.2026;
- чанк карты: `patient_misha/30-napravlenie-uzi-rebenok.md`.

**Борис.** «Какое направление у Анны Соколовой?»

```text
Эти сведения закрыты политикой доступа. Могу помочь с публичными услугами или вашей картой.
```

- `acl_denied=true`; чанк направления Анны в ответ не вошёл.

**Анна.** «Болит живот, что это за диагноз?»
...
Опора ответа Анны по сыну. Карточка Миши в справочный граф не входит.

```mermaid
flowchart LR
    Belova[Белова И.С. педиатр] -->|PROVIDES| PED[PED-01 приём]
    Morozova[Морозова А.П. УЗИ] -->|PROVIDES| USI2[USI-02 УЗИ ОБП ребёнку]
    ...
    Chunk[чанк направления Миши] -.->|subject_id=misha| USI2
```

Policy для Бориса: публичные узлы допускаются, чанк Анны отсекается.

```mermaid
flowchart LR
    Boris[actor boris] --> Pol[Policy]
    Pub[USI-01 K80.1 public] --> Pol
    Hidden[чанк направления Анны acl=patient] --> Pol
    Pol -->|допущено| Out[публичные услуги]
    Pol -->|отклонено| Deny[acl_denied]
```
