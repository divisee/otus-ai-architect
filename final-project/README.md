...
| [`docs/adr/`](docs/adr/README.md) | девять записей архитектурных решений: принятый вариант, отклонённые, последствия |
| [`slides/`](slides/) | схемы: исходники Mermaid (`*.mmd`) и собранные `*.svg`, обложка, QR-коды |
| [`presentation.html`](presentation.html) | презентация защиты, публикуется на GitHub Pages |
...
- [9. Внешние порты](#внешние-порты)
  - [9.1. Телефония](#телефония)
  - [9.2. CRM](#crm)
- [10. Схемы](#схемы)
  - [10.1. C4 L1 — Context](#c4-l1--context)
  - [10.2. C4 L2 — Container](#c4-l2--container)
  - [10.3. C4 L3 — Component](#c4-l3--component)
  - [10.4. Мультиагентная схема](#мультиагентная-схема)
  - [10.5. Когнитивная схема](#когнитивная-схема)
  - [10.6. Sequence](#sequence)
  - [10.7. Data Flow](#data-flow)
  - [10.8. Хранилища и очереди](#хранилища-и-очереди)
  - [10.9. Deployment](#deployment)
  - [10.10. Онтология и ER](#онтология-и-er)
- [11. Нагрузка и стоимость](#нагрузка-и-стоимость)
  - [11.1. Допущения](#допущения)
  - [11.2. Типовой день и пик](#типовой-день-и-пик)
  - [11.3. Капитальные затраты](#капитальные-затраты)
  - [11.4. Операционные затраты](#операционные-затраты-ориентир)
  - [11.5. Стоимость сессии](#стоимость-сессии)
- [12. Мониторинг промышленного контура](#мониторинг-промышленного-контура)
- [13. Метрики и проверка эффекта](#метрики-и-проверка-эффекта)
  - [13.1. Онлайн-метрики](#онлайн-метрики)
  - [13.2. Офлайн-метрики](#офлайн-метрики)
  - [13.3. A/B против живой линии](#ab-против-живой-линии)
  - [13.4. Экономия линии](#экономия-линии)
  - [13.5. Цели первого года](#цели-первого-года)
- [14. Планы развития](#планы-развития)
- [15. Реестр требований](#реестр-требований)
- [16. Стенд](#стенд)
  - [16.1. Запуск](#запуск)
  - [16.2. Развёртывание](#развёртывание)
  - [16.3. Порты стенда](#порты-стенда)
  - [16.4. Сценарий нагрузки для прогона](#сценарий-нагрузки-для-прогона)
  - [16.5. Локальный прогон](#локальный-прогон)
- [17. Презентация](#презентация)
- [18. Глоссарий терминов и сокращений](GLOSSARY.md)

## Зачем это клинике

Справочная линия двух филиалов — 500 обращений в день, в пиковый час около ста ([допущения расчёта](#допущения)). Одно справочное обращение занимает у оператора около четырёх минут ([Экономия линии](#экономия-линии)). Слева — состояние линии сегодня, справа — что даёт контур в периметре.
...
| SMS, почта | уведомление пациенту после подтверждения записи | время визита, филиал, услуга |
...
Отдельный вопрос защиты — что и где лежит, кроме графа и индекса. Всё перечисленное разворачивается внутри периметра; облачных сервисов, включая S3 и управляемые очереди, в контуре нет (ADR-0001).
...
| Подготовка обучающих данных | деперсонализация тем же анонимизатором, что в runtime; выборка, чекпоинты и обучение — внутри периметра, в сегменте GPU; версионирование и срок хранения | обучающие данные — это ПДн и врачебная тайна (152-ФЗ, 323-ФЗ ст. 13) |
...
| ------- | --------------- | ---------- |
| Стенд (MVP) | `docker compose --profile data up -d` + `uvicorn app.main:app` | Neo4j, Qdrant, PostgreSQL в контейнерах; Control Plane на хосте; корпус и граф загружаются при старте; `pytest -q` — 19 автотестов |
...
Terraform в поставку не входит: инфраструктура клиники — bare-metal в собственном периметре, описание кодом имеет смысл только если заказчик переводит контур в частное облако. Kubernetes — целевое состояние; на защите показывается локальный прогон.
...
Фаза GPU не блокирует проверку ACL и GraphRAG. Стенд: **19 тестов** (ACL, ReBAC, обязанности регистратуры, диагностика, HITL, оба канала, BM25).
...
Критерий стенда без GPU: 19 автотестов. Критерий целевого контура: p95 `/v1/chat` ≤ 8 с при 10 RPS на H100; p95 ASR реплики 5 с ≤ 1,0 с на L40S.

### Локальный прогон

Дата: **2026-09-12**. Хост: macOS, `uvicorn` на `127.0.0.1:8080`. GPU и vLLM не запускались. Генератор — grounded, граф — Cypher-seed в памяти, индекс — BM25. Автотесты: **19 passed**.
...
**Маршрут выступления.** Регламент защиты — пять минут, колода при этом остаётся полной: часть слайдов показывается только по вопросам. Звезда в правом верхнем углу отмечает слайд основного выступления. Отмечены одиннадцать: обложка, автор, содержание, цель и задачи, проблема и ценность (слайды 1–5); границы контура, управление диалогом и хранилища, мультиагентная система (8–10); результаты, выводы и контакты (31–33). Клавиши `Shift` + `→` и `Shift` + `←` переходят между отмеченными слайдами, обычные стрелки — по всем подряд.
...
| 7 | четыре уровня контура | [Стек](#стек) |
| 8–9 | C4 L1, L2 | [Схемы](#схемы) |
| 10 | мультиагентная система: субагенты, инструменты, HITL | [Мультиагентная схема](#мультиагентная-схема), [C4 L3 — Component](#c4-l3--component) |
| 11–12 | каналы и четыре ручки API | [Внешние порты](#внешние-порты), [Стенд](#стенд) |
| 13 | когнитивная схема пути реплики | [Когнитивная схема](#когнитивная-схема) |
| 14 | девять решений ADR | [Стек](#стек), [`docs/adr/`](docs/adr/README.md) |
| 15–17 | правило доступа, матрица доступа, запись с подтверждением | [Доступ](#доступ), [Sequence](#sequence) |
| 18–20 | онтология, ER, потоки данных | [Онтология и ER](#онтология-и-ер), [Data Flow](#data-flow) |
| 21 | четыре зоны сети | [Deployment](#deployment), [Хранилища и очереди](#хранилища-и-очереди) |
| 22 | расчёт нагрузки и выбор видеокарт | [Типовой день и пик](#типовой-день-и-пик), [ADR-0004](docs/adr/0004-gpu-budget.md) |
...
| 26–30 | контуры измерения, онлайн, офлайн, A/B, цели года | [Метрики и проверка эффекта](#метрики-и-проверка-эффекта) |
| 31 | результат работы | [Реестр требований](#реестр-требований), [Локальный прогон](#локальный-прогон) |
