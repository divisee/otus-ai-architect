...
**Маршрут выступления.** Регламент — пять минут, колода при этом остаётся полной: часть слайдов показывается только по вопросам. Звезда в правом верхнем углу отмечает слайд основного выступления. Отмечены шестнадцать: обложка, автор, содержание, цель и задачи, проблема и ценность (слайды 1–5); архитектура контура, управление диалогом и хранилища, мультиагентная система (7–9); матрица доступа (13); MVP против промышленного контура (21); стоимость владения и три контура метрик (24–25); результаты, выводы о работе, итог и контакты (28–31). Клавиши `Shift` + `→` и `Shift` + `←` переходят между отмеченными слайдами, обычные стрелки — по всем подряд.
...
| 10 | четыре метода API | [Внешние порты](#внешние-порты), [Стенд](#стенд) |
| 11 | когнитивная схема пути реплики | [Когнитивная схема](#когнитивная-схема) |
| 12 | десять решений ADR | [Стек](#стек), [`docs/adr/`](docs/adr/README.md) |
| 13 | матрица доступа к сведениям | [Доступ](#доступ) |
| 14 | обработка персональных данных в реплике | [Персональные данные в реплике](#персональные-данные-в-реплике), [ADR-0010](docs/adr/0010-pii-detection.md) |
| 15 | запись ребёнка с подтверждением | [Sequence](#sequence) |
| 16–18 | онтология, ER, потоки данных | [Онтология и ER](#онтология-и-er), [Data Flow](#data-flow) |
| 19 | четыре зоны сети | [Deployment](#deployment), [Хранилища и очереди](#хранилища-и-очереди) |
| 20 | расчёт нагрузки и выбор видеокарт | [Типовой день и пик](#типовой-день-и-пик), [ADR-0004](docs/adr/0004-gpu-budget.md) |
| 21 | MVP: что проверено на стенде и что будет в промышленном контуре | [Порты стенда](#порты-стенда), [Локальный прогон](#локальный-прогон) |
...
| 25 | три контура метрик: онлайн, офлайн, правовой | [Метрики и проверка эффекта](#метрики-и-проверка-эффекта) |
| 26–27 | A/B против живой линии, цели первого года | [A/B против живой линии](#ab-против-живой-линии), [Цели первого года](#цели-первого-года) |
| 28 | результат работы | [Реестр требований](#реестр-требований), [Локальный прогон](#локальный-прогон) |
| 29 | выводы о работе: цель, трудности, время, развитие | [Цель и задачи](#цель-и-задачи), [Планы развития](#планы-развития) |
| 30–31 | итог и контакты | весь документ |
