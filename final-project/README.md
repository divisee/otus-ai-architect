...
Ответ формулирует один из трёх исполнителей, переключается в интерфейсе: **Ollama** на этом же хосте (по умолчанию, если поднята; модель `qwen2.5:7b`, 4,7 ГБ на диске), **сборщик из репозитория** без сети и ключей или **OpenRouter**. Проверка прав и guardrails при любом выборе остаются в коде: отказ в диагнозе и отказ по доступу формулирует не модель. Внешний API допустим только на стенде, где данные синтетические; в промышленном контуре он запрещён ([ADR-0001](docs/adr/0001-closed-contour.md)).
...
Дата: **2026-09-20**. Хост: MacBook Pro, Apple M5 Pro, 48 ГБ, macOS 26.6; Python 3.14, `uvicorn` на `127.0.0.1:8080`, демонстрационный стенд на `8501`. GPU и vLLM не запускались. Генератор — grounded, граф — Cypher-seed в памяти, индекс — BM25. Автотесты: **29 passed**.
