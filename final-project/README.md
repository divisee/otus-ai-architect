...
- [1. Зачем это клинике](#зачем-это-клинике)
- [2. Цель и задачи](#цель-и-задачи)
- [3. Владение](#владение)
- [4. Объём системы](#объём-системы)
  - [4.1. Что входит](#что-входит)
  - [4.2. Ограничения](#ограничения)
  - [4.3. Принципы контура](#принципы-контура)
- [5. Роли](#роли)
- [6. Стек](#стек)
- [7. Доступ](#доступ)
  - [7.1. Правило Policy](#правило-policy)
  - [7.2. Карточки и граф](#карточки-и-граф)
- [8. Внешние порты](#внешние-порты)
  - [8.1. Телефония](#телефония)
  - [8.2. CRM](#crm)
- [9. Схемы](#схемы)
  - [9.1. C4 L1 — Context](#c4-l1--context)
  - [9.2. C4 L2 — Container](#c4-l2--container)
  - [9.3. C4 L3 — Component](#c4-l3--component)
  - [9.4. Когнитивная схема](#когнитивная-схема)
  - [9.5. Sequence](#sequence)
  - [9.6. Data Flow](#data-flow)
  - [9.7. Deployment](#deployment)
  - [9.8. Онтология и ER](#онтология-и-er)
- [10. Нагрузка и стоимость](#нагрузка-и-стоимость)
  - [10.1. Допущения](#допущения)
  - [10.2. Типовой день и пик](#типовой-день-и-пик)
  - [10.3. Капитальные затраты](#капитальные-затраты)
  - [10.4. Операционные затраты](#операционные-затраты-ориентир)
  - [10.5. Стоимость сессии](#стоимость-сессии)
- [11. Метрики и проверка эффекта](#метрики-и-проверка-эффекта)
  - [11.1. Онлайн-метрики](#онлайн-метрики)
  - [11.2. Офлайн-метрики](#офлайн-метрики)
  - [11.3. A/B против живой линии](#ab-против-живой-линии)
  - [11.4. Экономия линии](#экономия-линии)
  - [11.5. Цели первого года](#цели-первого-года)
- [12. Реестр требований](#реестр-требований)
- [13. Стенд](#стенд)
  - [13.1. Запуск](#запуск)
  - [13.2. Порты стенда](#порты-стенда)
  - [13.3. Сценарий нагрузки для прогона](#сценарий-нагрузки-для-прогона)
  - [13.4. Локальный прогон](#локальный-прогон)
- [14. Презентация](#презентация)
- [15. Глоссарий терминов и сокращений](GLOSSARY.md)

## Зачем это клинике

Справочная линия двух филиалов — 500 обращений в день, в пиковый час около ста ([допущения расчёта](#допущения)). Одно справочное обращение занимает у оператора около четырёх минут ([Экономия линии](#экономия-линии)).

- вопросы повторяются: цена, филиал, подготовка к исследованию, свободное окно;
- линия работает 10 часов в день, 250 дней в году; вне этого окна обращение не обрабатывается;
- непринятый звонок — несостоявшийся визит: пациент записывается там, где ответили (допущение, проверяется в A/B);
- облачный ассистент дешевле, но речь и карточки нельзя передавать внешнему сервису (152-ФЗ, 323-ФЗ ст. 13);
- целевое состояние: справку и запись ведёт ассистент в периметре, оператор остаётся на жалобе и исключении — до 40% обращений без оператора ([Цели первого года](#цели-первого-года)).
...
Первые три строки работают как ворота релиза: не прошли — выкладки нет. К выкладке прикладывается отчёт прогона: дата, версия корпуса и правил доступа, вердикт по каждому набору. Стенд без GPU закрывает их 15 автотестами (доступ, опека, диагноз, HITL, оба канала). Целевой контур добавляет Recall@10 и nDCG на bge-m3.
...
| 3 | зачем клинике голосовой ассистент | [Зачем это клинике](#зачем-это-клинике) |
| 4 | ценность для клиники | [Экономия линии](#экономия-линии), [Цели первого года](#цели-первого-года) |
| 5 | содержание | [Содержание](#содержание) |
| 6 | цель и задачи проекта | [Цель и задачи](#цель-и-задачи) |
| 7 | рамка контура и границы объёма | [Объём системы](#объём-системы) |
| 8–9 | четыре места контура, девять ADR | [Стек](#стек) |
| 10–12 | C4 L1, L2, L3 | [Схемы](#схемы) |
...
| 16–19 | правило Policy, три пациента, запись, отказ | [Доступ](#доступ), [Sequence](#sequence) |
| 20–23 | онтология, опора ответа, ER, потоки данных | [Онтология и ER](#онтология-и-ер), [Data Flow](#data-flow) |
| 24 | четыре зоны сети | [Deployment](#deployment) |
| 25 | ёмкость, TCO, локальный прогон | [Нагрузка и стоимость](#нагрузка-и-стоимость), [Локальный прогон](#локальный-прогон) |
| 26–30 | контуры измерения, онлайн, офлайн, A/B, цели года | [Метрики и проверка эффекта](#метрики-и-проверка-эффекта) |
| 31–36 | выводы, итог, контакты | весь документ |
