...
Идентификатор контура: `clinic-voice-graphrag`. Дата фиксации: **2026-09-20**. Обновлять при смене модели, Policy или состава GPU.
...
Разделение — не дань терминологии, оно окупается тремя вещами. **Первое: правила доступа проверяются без видеокарт.** Решение о доступе, отказ в диагнозе и подтверждение записи принимает Control Plane до обращения к модели, поэтому все 29 автотестов идут на ноутбуке за доли секунды, а GPU нужен только для задержки и качества формулировок ([Порты стенда](#порты-стенда)). **Второе: хранилище меняется, правила — нет.** Neo4j вместо графа в памяти, Qdrant вместо BM25, vLLM вместо стендового сборщика — это замена реализации за узким контрактом (`search`, `walk`, `generate`), и ни одно правило Policy при этом не переписывается. **Третье: границу видно в сети.** Плоскости разнесены по зонам ([Deployment](#deployment)): GPU-узлы принимают запросы только от Control Plane и наружу не ходят, а значит «модель не отправит данные во внешний API» — свойство размещения, а не обещание промпта ([ADR-0001](docs/adr/0001-closed-contour.md)).
...
| ------- | --------------- | ---------- |
| Стенд (MVP) | `docker compose --profile data up -d` + `uvicorn app.main:app` | Neo4j, Qdrant, PostgreSQL в контейнерах; Control Plane на хосте; корпус и граф загружаются при старте; `pytest -q` — 29 автотестов |
...
Стенд закрывает **29 тестами** проверку ACL, ReBAC, обязанностей регистратуры, отказа в диагнозе, HITL, обнаружения ПДн, обоих каналов и ранжирования BM25 — фаза GPU для этого не нужна.
...
Критерий стенда разработки: 29 автотестов. Критерий целевого контура: p95 `/v1/chat` ≤ 8 с при 10 RPS на H100; p95 ASR реплики 5 с ≤ 1,0 с на L40S.

### Локальный прогон

Дата: **2026-09-21**. Хост: MacBook Pro, Apple M5 Pro, 48 ГБ, macOS 26.6; Python 3.14, `uvicorn` на `127.0.0.1:8080`, демонстрационный стенд на `8501`. GPU и vLLM не запускались. Генератор — grounded, граф — Cypher-seed в памяти, индекс — BM25. Автотесты: **29 passed**.
