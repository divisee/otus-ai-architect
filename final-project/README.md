...
| ------- | --------------- | ---------- |
| Стенд (MVP) | `docker compose --profile data up -d` + `uvicorn app.main:app` | Neo4j, Qdrant, PostgreSQL в контейнерах; Control Plane на хосте; корпус и граф загружаются при старте; `pytest -q` — 28 автотестов |
...
Стенд закрывает **28 тестами** проверку ACL, ReBAC, обязанностей регистратуры, отказа в диагнозе, HITL, обнаружения ПДн, обоих каналов и ранжирования BM25 — фаза GPU для этого не нужна.
...
Критерий стенда разработки: 28 автотестов. Критерий целевого контура: p95 `/v1/chat` ≤ 8 с при 10 RPS на H100; p95 ASR реплики 5 с ≤ 1,0 с на L40S.

### Локальный прогон

Дата: **2026-09-20**. Хост: MacBook Pro, Apple M5 Pro, 48 ГБ, macOS 26.6; Python 3.14, `uvicorn` на `127.0.0.1:8080`, демонстрационный стенд на `8501`. GPU и vLLM не запускались. Генератор — grounded, граф — Cypher-seed в памяти, индекс — BM25. Автотесты: **28 passed**.
...
| 16 | четыре зоны сети | [Deployment](#deployment), [Хранилища и очереди](#хранилища-и-очереди) |
