...
- [12. Глоссарий терминов и сокращений](GLOSSARY.md)
...
| Идентификатор     | Права                                               |
| ----------------- | --------------------------------------------------- |
| пациент 1         | своя карта и карта пациента 2 (опека)               |
| пациент 2         | учётной записи нет; запросы выполняет представитель |
| пациент 3         | **только своя карта**                               |
...
Эффект считают в двух контурах. **Онлайн** — каждый рабочий день на живом трафике. **Офлайн** — на фиксированном наборе сценариев до выкладки и при смене корпуса. Юридический контур не измеряется процентами: речь, карты и генерация не покидают периметр (152-ФЗ, 323-ФЗ ст. 13).
...
| CSAT после цикла | удовлетворённость пациента по короткой шкале | SMS или IVR после визита не используют облачный текст запроса; оценка — балл и причина |
...
| ------- | ----- | -------- |
| Доступ представителя и соседа | пациент 1, пациент 2, пациент 3 | представитель видит карту ребёнка; сосед получает отказ |
...
Стенд без GPU: 15 автотестов (доступ, опека, диагноз, HITL, оба канала). Целевой контур добавляет Recall@10 / nDCG на bge-m3.
...
| ----------------- | ------------------- | ------------------- |
| пациент 1         | УЗИ пациента 2, код N28.1 | опека, ответ   |
| пациент 3         | направление пациента 1    | отказ          |
