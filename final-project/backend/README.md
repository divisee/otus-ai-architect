...
[`app/agents/crm_node.py`](app/agents/crm_node.py). Запись меняет данные клиники, поэтому
исполняется только после подтверждения человеком (HITL, [R10](../README.md#реестр-требований)).
Перенос и отмена подчинены тому же правилу; их веток в маршруте стенда пока нет
([§15](#15-известные-упрощения)).
...
Имена инструментов в контракте субагента и вызовы, которые за ними стоят на стенде
(сводная таблица субагентов — в [корневом документе](../README.md#мультиагентная-схема)):

| Инструмент контракта | Вызов на стенде | Состояние |
| -------------------- | --------------- | --------- |
| `graph_search` | `graph.search`, `graph.walk` | реализован |
| `bm25_search` | `vectors.search` | реализован |
| `vector_search` | — | целевой контур: плотные векторы bge-m3 |
| `list_slots` | `crm.list_slots` | реализован, без учёта услуги и филиала |
| `book` | `crm.book` | реализован, только после подтверждения |
| `reschedule` | — | ветки в маршруте нет |
| `cancel` | `crm.cancel` | метод есть, оркестратор не вызывает |
| `escalate_operator` | флаг `state.escalate` | реализован как признак состояния |
...
(43 публичных, 12 карточных, 8 для сотрудников), словарь индекса — 501 термин.
